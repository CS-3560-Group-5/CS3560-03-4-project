...
class UpdateInventoryScreen(tk.Frame):
    ...
    def _table(self):
        ...
        # Data rows — plain frames, no canvas
        for i, p in enumerate(PRODUCTS):
    ...
    def _apply_restock(self):
        ...
        for p in PRODUCTS:
            ...
            if add < 0:
                errors.append(f"{p['code']}: negative quantity"); continue
            if add == 0: continue
            new_count = p["count"] + add
            if new_count > p["max"]:
                errors.append(f"{p['code']}: exceeds max ({p['max']})"); continue
            p["count"] = new_count
        ...
        if not updated:
            self.status_label.configure(text="No changes made.", fg=TEXT_GRAY); return
